...
sub tunning_eth_ring {
    ...
    if ($buf) {
        $buf =~ s/\r|\n/\t/g;
        if ( $buf =~
m/.+?Pre-set maximums:\s+RX:\s+(\d+|n\/a).+?TX:\s+(\d+|n\/a).+?Current hardware settings:\s+RX:\s+(\d+|n\/a).+?TX:\s+(\d+|n\/a)/
          )
        {
            my $max_rx_ring = $1;
            my $max_tx_ring = $2;
            ...
            if (   ( $max_rx_ring > 0 )
                && ( $target_rx_ring > 0 )
                && ( $max_rx_ring > $target_rx_ring )
                && ( $cur_rx_ring != $target_rx_ring ) )
            {
                system
"/usr/sbin/ethtool -G ${eth} rx ${target_rx_ring} >/dev/null 2>&1";
                print "Set the rx ring of ${eth} to ${target_rx_ring}\n";
            }
            if (   ( $max_tx_ring > 0 )
                && ( $target_tx_ring > 0 )
                && ( $max_tx_ring > $target_tx_ring )
                && ( $cur_tx_ring != $target_tx_ring ) )
            {
                system
"/usr/sbin/ethtool -G ${eth} tx ${target_tx_ring} >/dev/null 2>&1";
...
sub enable_eth_rps_rfs {
    ...
    for my $eth ( "eth0", "eth1" ) {
        if ( -d "/sys/class/net/${eth}/queues/rx-0" ) {
            ...
            open my $fh, ">", "/sys/class/net/${eth}/queues/rx-0/rps_cpus"
              or die;
            print $fh $eth_cpu_mask_hex;
            close $fh;

            open $fh, ">", "/sys/class/net/${eth}/queues/rx-0/rps_flow_cnt"
              or die;
            print $fh $value;
            close $fh;

            open my $fh, ">", "/sys/class/net/${eth}/queues/tx-0/xps_cpus"
              or die;
            print $fh $eth_cpu_mask_hex;
            close $fh;
