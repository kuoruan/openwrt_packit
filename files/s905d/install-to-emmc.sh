...
1)
	...
	;;
2)
	FDTFILE="meson-gxl-s905d-phicomm-n1-thresh.dtb"
	;;
0)
	cat <<EOF
...
echo "EMMC 上共有 $p 个旧的分区会被删除" >/tmp/fdisk.script
...
xfs)
	mkfs.xfs -f -L EMMC_SHARED /dev/${EMMC_NAME}p4
	mount -t xfs /dev/${EMMC_NAME}p4 /mnt/${EMMC_NAME}p4
	;;
btrfs)
	mkfs.btrfs -f -L EMMC_SHARED -m single /dev/${EMMC_NAME}p4
	mount -t btrfs /dev/${EMMC_NAME}p4 /mnt/${EMMC_NAME}p4
	;;
f2fs)
	mkfs.f2fs -f -l EMMC_SHARED /dev/${EMMC_NAME}p4
